...
export default function OriginalStatements() {
  ...
  return (
    <Layout title="Original Statements" subtitle="Banking / Bank / Original Statements">
      <div style={s.breadcrumb}>Dashboard / Banking / Bank / Original Statements</div>

      <div style={s.pageHeader}>
        <div>
          <h2 style={s.pageTitle}>Bank Statement</h2>
          <p style={s.pageSub}>Read-only original records — entries cannot be modified</p>
        </div>
        <div style={s.lockedBadge}>🔒 Read Only</div>
      </div>
            ...
            {withBalance.length === 0 ? (
              ...
              withBalance.map((entry, i) => (
                <tr key={entry.id} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent' }}>
                  ...
                  <td style={{ ...s.td, maxWidth: 220, wordBreak: 'break-word' }}>{entry.bankReference || '—'}</td>
        ...
        <div style={s.overlay} onClick={() => setViewModal(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.modalHeader}>
              <div style={s.modalTitle}>Transaction Details</div>
              <button style={s.closeBtn} onClick={() => setViewModal(null)}>✕</button>
            </div>
            <div style={s.modalGrid}>
              <ModalRow label="Account" val={viewModal.account} />
              <ModalRow label="Transaction Date" val={viewModal.transactionDate} />
              <ModalRow label="Bank Description" val={viewModal.bankDescription} span />
              <ModalRow label="Bank Reference" val={viewModal.bankReference || '—'} span />
              <ModalRow label="Debit Amount" val={`₹ ${formatINR(viewModal.debitAmount)}`} color="#f43f5e" />
              <ModalRow label="Credit Amount" val={`₹ ${formatINR(viewModal.creditAmount)}`} color="#10b981" />
            </div>
            <div style={s.modalNote}>🔒 This is a read-only original record and cannot be modified.</div>
          </div>
        </div>
      )}
    </Layout>
...
function ModalRow({ label, val, color, span }) {
  return (
    <div style={{ gridColumn: span ? '1/-1' : undefined }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.7px', marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 14, color: color || 'var(--text-primary)', fontWeight: color ? 700 : 400 }}>{val}</div>
    </div>
  );
}
...
const s = {
  ...
  lockedBadge: {
    ...
    border: '1px solid rgba(244,63,94,0.25)', alignSelf: 'flex-start',
  },
  filterBar: {
