// PATH: frontend/src_consultancy/pages/Permissions/PermissionsTransferred.jsx
...
const PermissionsTransferred = () => {
  const role = sessionStorage.getItem('consultancyUserRole');

  if (role === 'director') return <Navigate to="/consultancy-office/permissions/list" replace />;

  const [queue] = useState(() => loadQueue());
  const [filters, setFilters] = useState({});

  const allRows = getTransferredRows(queue, 'permissions', role);
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', margin: '0 0 4px' }}>Permissions — Transferred</h1>
...
export default PermissionsTransferred;