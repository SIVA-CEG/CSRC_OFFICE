// PATH: frontend/src_consultancy/pages/ProformaInvoice/ProformaInvoiceList.jsx
...
const COLUMNS = [
  ...
  { key: 'acfHcDate', label: 'ACF HC Date' },
];
...
const ProformaInvoiceList = () => {
  ...
  const allRows = getListRows(queue, 'proforma-invoices', role);
  const rows = useMemo(() => allRows.filter((r) =>
    !filters.q || `${r.id} ${r.consultantName} ${r.invoiceNo}`.toLowerCase().includes(filters.q.toLowerCase())
  ), [allRows, filters]);

  const deptCampusOptions = [...new Set(allRows.map((r) => r.deptCampus))];

  const handleSubmit = (row, action, extra) => {
    const next = submitAction(queue, 'proforma-invoices', row.id, role, action, extra);
    setQueue(next);
    saveQueue(next);
  };
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', margin: '0 0 4px' }}>Submitted Invoice Forms</h1>
      <p style={{ fontSize: 13.5, color: '#6b7280', margin: '0 0 18px' }}>
        Review proforma invoices and route them to Sanction Proceedings, or reject back to faculty.
      </p>
