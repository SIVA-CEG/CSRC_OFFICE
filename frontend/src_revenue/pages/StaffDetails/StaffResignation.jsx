...
export default function StaffResignation() {
  ...
  return (
    <div style={styles.root}>
      ...
      {isApproverRole(actor.role) ? <ApprovalQueue actor={actor} /> : <ResignationFlow actor={actor} />}
    </div>
  );
}

/* ------------------------------------------------------------------ */
/* Assistant view — select staff, then set resignation details          */
/* ------------------------------------------------------------------ */
function ResignationFlow({ actor }) {
  const [staff, setStaff] = useState([]);
  const [search, setSearch] = useState('');
  const [selected, setSelected] = useState(null);
  const [date, setDate] = useState('');
  const [docs, setDocs] = useState({ resignationLetter: null, relievingLetter: null, experienceLetter: null });
  const [error, setError] = useState('');
  const [submitted, setSubmitted] = useState(false);

  useEffect(() => { setStaff(getStaffList()); }, []);

  const eligible = useMemo(() => staff.filter((s) => s.status !== 'resigned'), [staff]);
  ...
  const pick = (s) => {
    setSelected(s);
    setDate('');
    setDocs({ resignationLetter: null, relievingLetter: null, experienceLetter: null });
    setError('');
  };
  ...
  const handleSubmit = () => {
    if (!date) { setError('Please select the resignation effective date.'); return; }
    if (!docs.resignationLetter || !docs.relievingLetter || !docs.experienceLetter) {
      setError('Resignation Letter, Relieving Letter and Experience Letter are all required.');
      return;
    }
    setError('');
    submitRequest(
      'resignation',
      { staffId: selected.id, staffName: fullName(selected), resignationDate: date },
      docs,
      actor
    );
    setSubmitted(true);
      ...
      <div className="sd-fade-in">
        ...
        <div style={styles.searchWrap}>
          ...
          <input
            ...
            placeholder="Search staff who are resigning..."
            value={search}
          ...
          {filtered.map((s) => {
            ...
            return (
              <div key={s.id} className="sd-row-hover" style={styles.staffRow} onClick={() => pick(s)}>
                ...
                <div style={{ flex: 1.2, fontSize: 13, color: theme.textMuted }}>Joined {formatDate(s.doj)}</div>
                <div style={{ flex: 1 }}>
                  <span style={{ ...styles.statusPill, color: meta.color, background: meta.bg }}>{meta.label}</span>
                </div>
                <div style={{ color: theme.rose, fontWeight: 700, fontSize: 13 }}>Process →</div>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div style={styles.formCard} className="sd-fade-in">
      ...
      <p style={styles.formSub}>{selected.designation} · Joined {formatDate(selected.doj)}</p>

      {error && <div style={styles.errorBanner}>{error}</div>}

      <div style={styles.fieldGrid}>
        <div style={styles.field}>
          <label style={styles.fieldLabel}>Resignation Effective Date</label>
          <input className="sd-input" style={styles.input} type="date" value={date} onChange={(e) => setDate(e.target.value)} />
        </div>
      </div>
      ...
      <div style={{ ...styles.group, marginTop: 22 }}>
        ...
        <div style={styles.uploadGrid}>
          <UploadBox label="Resignation Letter" file={docs.resignationLetter} onChange={(f) => handleFile('resignationLetter', f)} accent={theme.rose} />
          <UploadBox label="Relieving Letter" file={docs.relievingLetter} onChange={(f) => handleFile('relievingLetter', f)} accent={theme.rose} />
          <UploadBox label="Experience Letter" file={docs.experienceLetter} onChange={(f) => handleFile('experienceLetter', f)} accent={theme.rose} />
        </div>
      </div>
...
function ApprovalQueue({ actor }) {
  ...
  const refresh = () => setRequests(getRequestsByType('resignation'));
  useEffect(() => { refresh(); }, []);
  ...
  const open = (r) => { setSelected(r); setEditDate(r.data.resignationDate); setComment(''); };
  const close = () => setSelected(null);

  const handleApprove = () => {
    approveRequest(selected.id, actor, { resignationDate: editDate }, comment || undefined);
          ...
          <div style={styles.drawer} className="sd-scroll" onClick={(e) => e.stopPropagation()}>
            ...
            {selected.status === `pending_${actor.role}` ? (
              <div style={styles.fieldGrid}>
                <div style={styles.field}>
                  <label style={styles.fieldLabel}>Resignation Effective Date</label>
                  <input className="sd-input" style={styles.input} type="date" value={editDate} onChange={(e) => setEditDate(e.target.value)} />
                </div>
              </div>
            ) : (
              <div style={styles.fieldGrid}>
                <StaticField label="Resignation Effective Date" value={formatDate(selected.data.resignationDate)} />
              </div>
            )}

            <div style={styles.docsRow}>
              <DocChip label="Resignation Letter" doc={selected.documents?.resignationLetter} />
              <DocChip label="Relieving Letter" doc={selected.documents?.relievingLetter} />
              <DocChip label="Experience Letter" doc={selected.documents?.experienceLetter} />
            </div>
...
function RequestRow({ req, onClick, highlight }) {
  return (
    <div className="sd-row-hover" style={{ ...styles.staffRow, ...(highlight ? styles.requestRowHighlight : {}) }} onClick={onClick}>
      ...
      <div style={{ flex: 1.4, fontSize: 13, color: theme.textMuted }}>Effective {formatDate(req.data.resignationDate)}</div>
...
const styles = {
  ...
  requestRowHighlight: { background: theme.roseLight },
  ...
  group: {},
  groupTitle: { fontSize: 12, fontWeight: 800, color: theme.rose, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 14, paddingBottom: 8, borderBottom: `1px solid ${theme.border}` },
  uploadGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 },
  ...
  submitBtn: { border: 'none', background: theme.rose, color: '#fff', padding: '14px 28px', borderRadius: theme.radiusMd, fontWeight: 800, fontSize: 14.5, marginTop: 22 },
  ...
  docChip: { display: 'flex', alignItems: 'center', gap: 8, background: theme.bgAlt, borderRadius: theme.radiusSm, padding: '10px 14px', flex: 1, minWidth: 160 },
  ...
  approveBtn: { flex: 1, border: 'none', background: theme.rose, color: '#fff', padding: '12px', borderRadius: theme.radiusSm, fontWeight: 800, fontSize: 13.5 },
