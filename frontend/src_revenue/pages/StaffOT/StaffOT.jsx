...
import { getOTSanctionPendingCount, getAllOTReports } from '../../utils/otWorkflow';
import { theme, fontFaceAndUtilities } from '../../utils/theme';

const CARDS = [
  {
    key: 'structure',
    path: '/revenue/staff-ot/structure',
    title: 'OT Salary Structure',
    subtitle: 'Set the overtime rate per hour for every designation',
    icon: '🏗️',
    color: theme.indigo, colorDark: theme.indigoDark, bg: theme.indigoLight,
    badge: false,
  },
  {
    key: 'details',
    path: '/revenue/staff-ot/details',
    title: 'OT Details',
    subtitle: 'Enter daily In/Out time and overtime hours for each staff member',
    icon: '🕒',
    color: theme.amber, colorDark: theme.amberDark, bg: theme.amberLight,
    badge: false,
  },
  {
    key: 'sanction',
    path: '/revenue/staff-ot/sanction',
    title: 'OT Sanctions',
    subtitle: 'Review month-end OT totals per staff and route for approval',
    icon: '🧮',
    color: theme.emerald, colorDark: theme.emeraldDark, bg: theme.emeraldLight,
    badge: true,
  },
  {
    key: 'reports',
    path: '/revenue/staff-ot/reports',
    title: 'OT Reports',
    subtitle: 'Browse every past and present OT sanction, and export as PDF',
    icon: '📊',
    color: theme.rose, colorDark: theme.roseDark, bg: theme.roseLight,
    badge: false,
  },
];
...
export default function StaffOT() {
  ...
  useEffect(() => {
    ...
    if (isApproverRole(a.role)) {
      setPendingSanctions(getOTSanctionPendingCount(a.role));
    }
    const reports = getAllOTReports();
    setTotals({
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.header} className="sd-fade-in">
        <div>
          <div style={styles.eyebrow}>CSRC REVENUE · OT MODULE</div>
          <h1 style={styles.title}>Staff OT</h1>
          <p style={styles.subtitle}>
            Set overtime rates, log daily hours, sanction monthly OT and browse every report — all in one place.
          </p>
        </div>
...
const styles = {
  ...
  actorPill: { display: 'flex', alignItems: 'center', gap: 10, background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 999, padding: '8px 16px 8px 12px', boxShadow: theme.shadowSm },
  ...
  card: { position: 'relative', textAlign: 'left', background: theme.surface, border: '1px solid', borderRadius: theme.radiusLg, padding: 24, cursor: 'pointer', boxShadow: theme.shadowSm, fontFamily: theme.fontBody },
