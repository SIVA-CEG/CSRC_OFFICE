...
const PROJECTS = [
  { id: "CMR001", title: "AI Based Research Project",   pi: "Dr. Kumar", department: "IT",  sanctionedAmount: 500000 },
];

const INITIAL_CLAIMS = [
  { ref:"CMR001-CLM001", projectId:"CMR001", date:"01 Jun 2025", section:"A", title:"Non-Recurring Heads", head:"Equipment 1", amount:45000,  fileName:"invoice_eq1.pdf",  fileURL:null, status:"pending",  note:"" },
];
...
function StatusBadge({ status }) {
  ...
  return (
    <span className="cmrg-status-badge"
...
function ReviewDrawer({ claim, project, onClose, onDecide }) {
  ...
  return (
    <div className="cmrg-drawer-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="cmrg-drawer">
        ...
        <div className="cmrg-drawer-head">
          ...
          <button className="cmrg-drawer-close" onClick={onClose}>✕</button>
        </div>

        {/* Project strip */}
        <div className="cmrg-drawer-project-strip">
          <div className="cmrg-dps-item"><span>Project</span><b>{project?.id}</b></div>
          <div className="cmrg-dps-item"><span>PI</span><b>{project?.pi}</b></div>
          <div className="cmrg-dps-item"><span>Dept</span><b>{project?.department}</b></div>
          <div className="cmrg-dps-item"><span>Sanctioned</span><b>{fmt(project?.sanctionedAmount)}</b></div>
        </div>
        ...
        <div className="cmrg-drawer-section">
          ...
          <div className="cmrg-detail-grid">
            <div className="cmrg-detail-item"><label>Date Filed</label><span>{claim.date}</span></div>
            <div className="cmrg-detail-item"><label>Section</label>
              <span className="cmrg-section-pill">{claim.section}</span></div>
            <div className="cmrg-detail-item"><label>Head Category</label><span>{claim.title}</span></div>
            <div className="cmrg-detail-item"><label>Item / Equipment</label><span>{claim.head}</span></div>
          ...
          {claim.fileURL ? (
            <div className="cmrg-bill-file-card">
              ...
              <div className="cmrg-bill-file-actions">
                <button className="cmrg-file-btn preview"
                  onClick={() => window.open(claim.fileURL, "_blank")}>👁 Preview</button>
                <a className="cmrg-file-btn download"
        ...
        {/* Office note */}
        <div className="cmrg-drawer-section">
          <h4 className="cmrg-drawer-section-title">📝 Office Note / Remarks</h4>
          <textarea className="cmrg-note-input"
...
function HistoryModal({ claims, projects, onClose, onPreview }) {
  ...
  return (
    <div className="cmrg-modal-overlay" onClick={e => { if (e.target===e.currentTarget) onClose(); }}>
      <div className="cmrg-modal">
        {/* Modal header */}
        <div className="cmrg-modal-head">
          <div>
            <h2 className="cmrg-modal-title">📚 Full Claims History</h2>
            <p className="cmrg-modal-sub">{filtered.length} records · all projects</p>
          </div>
          <button className="cmrg-drawer-close" onClick={onClose}>✕</button>
        </div>
        ...
        <div className="cmrg-modal-filters">
          <input className="cmrg-filter-input" placeholder="🔍 Search ref, head, item…"
            value={search} onChange={e => setSearch(e.target.value)}/>
          <select className="cmrg-filter-select"
        ...
        <div className="cmrg-modal-summary">
          <div className="cmrg-ms-card green"><p>Approved Value</p><h4>{fmt(totals.approved)}</h4></div>
          <div className="cmrg-ms-card red"><p>Rejected Value</p><h4>{fmt(totals.rejected)}</h4></div>
          <div className="cmrg-ms-card yellow"><p>Pending Count</p><h4>{totals.pending}</h4></div>
        </div>

        {/* Table */}
        <div className="cmrg-modal-table-wrap">
          {filtered.length === 0 ? (
            <div className="cmrg-empty"><div className="cmrg-empty-icon">🗂️</div>No matching records found.</div>
          ) : (
                ...
                {filtered.map((c, i) => {
                  ...
                  return (
                    <tr key={c.ref}>
                      ...
                      <td><span className="cmrg-ref-badge">{c.ref}</span></td>
                      <td>
                      ...
                      <td><span className="cmrg-sec-badge">{c.section}</span></td>
                      <td>
                      ...
                      <td className="cmrg-note-cell">{c.note || <span className="cmrg-muted">—</span>}</td>
                      <td>
                        {c.fileURL
                          ? <button className="cmrg-file-btn preview" onClick={() => onPreview(c)}>👁</button>
...
const stats = useMemo(() => ({
  ...
  <header className="cmrg-nav">
    ...
    <div className="cmrg-nav-right">
      ...
      <div className="cmrg-officer-chip">
        <div className="cmrg-officer-avatar">
          OF
        </div>
  ...
  <div>
    ...
    <div className="cmrg-header-eyebrow">
...
<div className="cmrg-table-card">
                ...
                <table className="cmrg-claims-table">
                  ...
                  <tbody>
                    {visible.map((claim, i) => {
                      ...
                      return (
                        <tr key={claim.ref} className={`cmrg-tr-${claim.status}`}>
                          <td className="cmrg-row-num">{i+1}</td>
                          <td><span className="cmrg-ref-badge">{claim.ref}</span></td>
                          <td>
                            <div className="cmrg-proj-cell">
                          ...
                          <td><span className="cmrg-sec-badge">{claim.section}</span></td>
                          <td>
                            <div className="cmrg-head-cell">
                          ...
                          <td className="cmrg-amount-cell">{fmt(claim.amount)}</td>
                          <td>
                            {claim.fileURL
                              ? <button className="cmrg-file-btn preview"
      ...
      {/* ── File Preview ────────────────────────────────────────────── */}
      {pdfPreview && (
        <div className="cmrg-pdf-overlay"
