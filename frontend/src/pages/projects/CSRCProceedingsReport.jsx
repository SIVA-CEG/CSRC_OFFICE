...
const calcHeads = (inst) => {
  ...
  const nr =
    equipmentItems.length > 0
      ? equipmentSum
      : parseFloat(inst.nonRecurringTotal) || 0;
  const man =
    manpowerItems.length > 0 ? manpowerSum : parseFloat(inst.manpower) || 0;
...
export function assembleReportData(
  ...
  const piName =
    profile.name || endorsement.piName || "Dr. S. Balasivanandha Prabu";
  const piDesignation =
    profile.designation || endorsement.piDesignation || "Professor";
  const piDept =
    profile.department ||
    endorsement.piDept ||
  ...
  const projectTitle =
    endorsement.title || sanctionedForm.projectTitle || "Untitled Project";
  const fundingAgency =
    endorsement.fundingAgency ||
    sanctionedForm.fundingAgency ||
    "Funding Agency";
  ...
  const proceedingNo =
    sanctionedForm.proceedingNo || `CSRC/CTDT/${new Date().getFullYear()}/OBS`;
  const proceedingDate = sanctionedForm.proceedingDate || todayStr();

  const sanctionRef =
    sanctionedForm.sanctionRef || `${proceedingNo}, dated ${proceedingDate}`;

  const installmentNumber = instIndex + 1;
  const instLabel =
    inst.label ||
    inst.installmentNo ||
    `${ordinal(installmentNumber)} Installment`;
  ...
  const bankAccount =
    sanctionedForm.bankAccount || profile.accountNumber || "123456789012";

  const ifsCode = sanctionedForm.ifscCode || profile.ifscCode || "SBIN0006756";

  const bankBranch =
    sanctionedForm.bankBranch || profile.bankBranch || "Anna University Branch";
  ...
  const previousInstallments =
    previousInstallmentsOverride && previousInstallmentsOverride.length > 0
      ? previousInstallmentsOverride
      : allInstallments.slice(0, instIndex).map((pInst, idx) => ({
          label:
            pInst.label ||
            pInst.installmentNo ||
            `${ordinal(idx + 1)} Installment`,
...
function PrevInstallmentsTable({ previousInstallments }) {
  ...
  return (
    <div style={S.tableWrap}>
      <p
        style={{ fontWeight: "bold", marginBottom: "4px", fontSize: "10.5pt" }}
      >
        ...
        <tbody>
          {previousInstallments.map((pi, i) => (
            <tr key={i}>
              ...
              <td style={S.tdCenter}>
                {pi.releasedDate
                  ? new Date(pi.releasedDate).toLocaleDateString("en-GB")
                  : "—"}
              </td>
            </tr>
          ))}
...
function BudgetTable({ data }) {
  ...
  const rows = [
    ...
    {
      sl: "5",
      head: "i) The Registrar A/C, Chennai 5%",
      amount: heads.oh.registrar,
      group: false,
      indent: true,
    },
    {
      sl: "6",
      head: "ii) CSRC Revenue, Chennai 4%",
      amount: heads.oh.csrc,
      group: false,
      indent: true,
    },
    {
      sl: "7",
      head: "iii) The Dean, Campus A/C 4%",
      amount: heads.oh.dean,
      group: false,
      indent: true,
    },
    {
      sl: "8",
      head: "iv) The Principal Investigator PDF 2%",
      amount: heads.oh.pdf,
      group: false,
      indent: true,
    },
      ...
      <table style={S.table}>
        ...
        <tbody>
          ...
          <tr style={S.totalRow}>
            <td
              colSpan={2}
              style={{ ...S.td, textAlign: "right", fontWeight: "bold" }}
            >
              Total Amount
            </td>
...
//   - isCompleted=false → all 4 slots; signed = image/✔, unsigned = "Pending"
...
function SignatureBlock({ signatures = {}, isCompleted = false }) {
  ...
  const renderSig = (sig) => {
    if (!sig) {
      return <div style={sigStyle.pending}>Pending</div>;
    }
    ...
    return (
      <div style={sigStyle.wrap}>
        <div style={sigStyle.item}>
          ...
          {signatures.director && (
            <div style={sigStyle.checkmark}>✔ Signed</div>
          )}
        </div>
      </div>
    );
  ...
  // Workflow mode: all 4 slots (assistant → superintendent → dd → director)
  const slots = [];

  if (signatures.assistant)
    slots.push({
      key: "assistant",
      label: "ASST. (Office)",
    });

  if (signatures.superintendent)
    slots.push({
      key: "superintendent",
      label: "SUPDT",
    });
...
export default function CSRCProceedingsReport({
  reportData,
  signatures = {},
  isCompleted = false,
}) {
  ...
  const amountInWords = numberToWords(d.heads.grand);
  return (
    <div style={S.page}>
      {/* ── Letterhead ── */}
      <div style={S.header}>
      ...
      <div style={S.proceedingRow}>
        <span>
          <strong>Proceedings No: </strong>
          {d.proceedingNo}
        </span>
        <span>
          <strong>Date: </strong>
          {fmtDate(d.proceedingDate) || todayStr()}
        </span>
      </div>
      ...
      <div style={{ fontSize: "11pt", marginBottom: "4px" }}>
        ...
        <ol style={{ margin: "2px 0 2px 18px", padding: 0 }}>
          <li style={{ marginBottom: "2px" }}>
            Anna University — SERB — SRC — Syndicate Resolution No. 172 S.D. dt.
            28.12.2016 —&nbsp;
      ...
      <p style={S.subject}>
        ...
        {d.piDept}, {d.piCampus} as the Principal Investigator for the
        project&nbsp;
        {d.projectPeriod && (
          <>
            for the period <strong>{d.projectPeriod}</strong>
          </>
        )}
      ...
      {/* ── Body ── */}
      <p style={S.para}>
        The Science and Engineering Research Board, New Delhi has sanctioned a
        project entitled&nbsp;
        <strong>"{d.projectTitle}"</strong>&nbsp; under{" "}
        <strong>"{d.projectScheme || d.fundingAgency}"</strong>&nbsp; to{" "}
        <strong>{d.piName}</strong>,&nbsp;
        <strong>{d.piDesignation}</strong>,&nbsp;
        <strong>{d.piDept}</strong>, {d.piCampus}, as the Principal Investigator
        for the above said project.&nbsp; The funding agency has already been
        released by the funding agency and the necessary sanction proceedings
        had also been issued for the implementation of the above said project,
      ...
      <p style={S.para}>
        ...
        <strong>{d.instLabel}</strong>&nbsp; of Rs.{" "}
        <strong>{fmtINR(d.heads.grand)}</strong>&nbsp; (Rupees{" "}
        <em>{d.heads.grand ? amountInWords : "___________"}</em>) vide the
        reference third cited above.
      </p>

      <p style={S.para}>
        In the reference fourth cited above, <strong>{d.piName}</strong>,
        Principal Investigator of the Project,&nbsp; has requested to transfer
        the Project grant to The Director after deducting Overhead charges as
        per the CSRC norms.
      </p>
      ...
      <p style={S.para}>
        ...
        ,&nbsp; to meet the expenses related to the above project, after
        deducting Overhead charges as per the CSRC norms.
      </p>

      <p style={S.para}>
        In view of the above and as per the powers delegated in the reference
        first cited above, sanction is hereby accorded&nbsp; to transfer an
        amount not exceeding&nbsp;
        <strong>Rs. {fmtINR(d.heads.grand)}</strong>&nbsp; ({d.instLabel}) being
        the Project grant and Overhead charges to the respective account, as
        detailed below.
      </p>
      ...
      {/* ── Notes ── */}
      <p style={S.note}>
        The CSRC overhead charges 6(ii) along with PDF amount 8(iv) are retained
        in the Revenue a/c of CSRC.
      </p>
      <p style={S.note}>
        The amount allocated under PDF a/c... can be utilized by the individual,
        as per the norms prescribed in the CSRC Guidelines.
      </p>
      <p style={S.note}>
        The expenditure for the above project will be debitable to T/F – SERB
        Project&nbsp;
        <strong>"{d.projectTitle}"</strong>&nbsp; by <strong>{d.piName}</strong>
        , <strong>{d.piDesignation}</strong>,&nbsp;
        {d.piDept}, {d.piCampus}.
      </p>
      <p style={S.note}>
        The above sanction has been entered in the Project Sanction Register
        Vol. · VIIIB vide Sl. No. 2 at Page No. 98.
      </p>
      <p style={S.note}>
        The expenditure in this regard is debitable from{" "}
        <strong>"CSRC Project Account"</strong>&nbsp; under the Head of account
        "Project".
      </p>

      <p style={S.para}>
        The items and consumables as detailed in your project may be purchased
        by following the University guidelines / rules in force at Anna
        University. The utilization of Contingency and Travel shall be as per
        the rules and regulations specified by Anna University.
      </p>

      {/* ── Signature block (4-stage or director-only) ── */}
